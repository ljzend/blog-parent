      ...
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="用户昵称" align="center" key="nickName" prop="nickName" v-if="columns[2].visible"
                           :show-overflow-tooltip="true"
          />
          <el-table-column label="手机号码" align="center" key="phone" prop="phone" v-if="columns[3].visible"/>
...
<script>
import { listUser, addUser, deleteUser, updateUser, changeUserStatus, resetPwd } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      ...
      // 状态常量数据
      statusCode: [{ value: '0', label: '正常' }, { value: '1', label: '停用' }],
      defaultProps: {
        children: 'children',
        label: 'label'
      },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.myConfirm('确认要"' + text + '""' + row.userName + '"用户吗？').then(function() {
    ...
    handleAdd() {
      this.reset()
      getRolesByUserId().then(response => {
    ...
    handleDelete(row) {
      ...
      this.myConfirm('是否确认删除用户编号为"' + userIds + '"的数据项？').then(function() {
